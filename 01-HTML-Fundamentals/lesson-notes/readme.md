...
* **🎯 Ödev 1 (Kişisel Profil ve İlgi Alanları Sayfası):**
    ...
    * **🎯 Öğrencilerden Beklenen Kriterler (Ödev Görevleri):**
        1. **Semantik İskelet Mimari:** Tüm sayfalarda `<header>`, `<nav>`, `<section>`, `<article>` ve `<footer>` etiketleri hiyerarşik olarak eksiksiz ve doğru sırayla kullanılmalıdır.
        2. **Gelişmiş Navigasyon Kontrolü:** Sayfalar arasında kusursuz geçiş sağlayan menü linkleri (`<a>`) hem `header` hem de `footer` alanlarında doğru dosya yollarına yönlendirilmelidir.
        3. **İçerik ve Hiyerarşi Yönetimi:** Ana sayfada başlıklar (`<h2>`) ve yatay çizgilerle (`<hr>`) birbirinden ayrılmış en az 3 bağımsız `<article>` bloğu kurgulanmalıdır.
        4. **Görsel Entegrasyonu:** `picsum.photos` üzerinden dinamik mock-up görseller çekilmeli ve erişilebilirlik (accessibility) standartları gereği tüm `<img>` etiketlerinde anlamlı `alt` nitelikleri tanımlanmalıdır.
        5. **Çift Liste Mantığı:** Proje kapsamında hem sıralı liste (`<ol>`) hem de sırasız liste (`<ul>`) yapılarına yer verilerek veri listeleme mantığı kavranmalıdır.
        6. **İletişim Protokolleri:** İletişim sayfasında kurumsal bilgiler `<address>` etiketi içine alınmalı; telefon numarası için `tel:`, e-posta adresi için `mailto:` protokolleri aktif edilerek tıklanabilir hale getirilmelidir.

    * **✅ Kazanımlar:** Çok sayfalı web sitesi mimarisi tasarımı, SEO dostu semantik kurgu yeteneği, dosya yolları (`relative path`) yönetimi ve temel HTML element hiyerarşisine tam hakimiyet.
---
